Course 1.3 Photo Gallery - Add About Section
============================================

## 1.3.1 Add the About Section 

![lesson 3.1 screenshot](screenshots/3.1.png)

* create the about section
* add the h2 headline
* center and style the headline

```html
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Photo Gallery</title>
    <meta name="description" content="Some beautiful stock photography"/>
    <link href="https://fonts.googleapis.com/css?family=Montserrat:300,400,500,600&display=swap" rel="stylesheet">
    <style>
        body, html{
            margin: 0;
            padding: 0;
            font-family: 'Montserrat', sans-serif;
        }

        .hero{
            position: relative;
        }

        .hero-image{
            width: 100%;
            height: auto;
        }

        .hero-text{
            position: absolute;
            right: 0;
            bottom: 0;
            left: 0;
            padding: 5rem 2.5rem;
            text-align: center;
            color: #fff;
            background-image: linear-gradient(rgba(0,0,0,0), rgba(0,0,0,.5));
        }

        .hero-text h1{
            font-weight: 300;
            font-size: 3rem;
            margin: 0 0 .5rem 0;
        }

        .slides{
            background: #424242;
            padding: 4px;
            display: flex
        }

        .slide{
            flex: 1;
            margin: 8px;
            border-radius: 4px;
            overflow: hidden;
            box-shadow: 0.3em 0.3em 1em rgba(0,0,0,0.3);
        }

        .slide img{
            width: 100%;
            height: auto;
        }

        .about{
            max-width: 1040px;
            margin: 0 auto;
        }

        .about h2{
            font-weight: 300;
            font-size: 2.5rem;
            margin: 2.5rem 0;
            text-align: center;
        }
    </style>
</head>
<body>
    <section class="hero">
        <img id="hero-image" class="hero-image" src="https://picsum.photos/seed/hero/1040/780">
        <div class="hero-text">
            <h1>Awesome Photo Gallery</h1>
            <p class="lead">Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
        </div>
    </section>
    <section class="slides">
        <div class="slide" onclick="switchImage('https://picsum.photos/seed/slide1')">
            <img src="https://picsum.photos/seed/slide1/240">
        </div>
        <div class="slide" onclick="switchImage('https://picsum.photos/seed/slide2')">
            <img src="https://picsum.photos/seed/slide2/240">
        </div>
        <div class="slide" onclick="switchImage('https://picsum.photos/seed/slide3')">
            <img src="https://picsum.photos/seed/slide3/240">
        </div>
        <div class="slide" onclick="switchImage('https://picsum.photos/seed/slide4')">
            <img src="https://picsum.photos/seed/slide4/240">
        </div>
        <div class="slide" onclick="switchImage('https://picsum.photos/seed/slide5')">
            <img src="https://picsum.photos/seed/slide5/240">
        </div>
    </section>
    <section class="about">
        <h2>About Us</h2>
    </section>
    <script type="text/javascript">
        // preload images to avoid delay when you switch the src
        for (let i = 1; i <= 5; i++) {
            const image = new Image();
            image.src = 'https://picsum.photos/seed/slide' + i + '/1040/780';
            console.log('pre-loading ' + image.src);
        }
        
        const switchImage = (image) => {
            // get a reference to the hero image element
            const hero = document.getElementById("hero-image");

            // now load a full resolution version  
            const large = image + '/1040/780';

            // and set the hero image src
            hero.src = large;
        }
    </script>
</body>
</html>
```

## 1.3.2 Add the about blocks

![lesson 3.2 screenshot](screenshots/3.2.png)

* add three blocks with an image and lorem ipsum
* make the image float with flex
* style the content
* center the content vertically with flex column
* make the blocks alternate with the image right and left

```html
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Photo Gallery</title>
    <meta name="description" content="Some beautiful stock photography"/>
    <link href="https://fonts.googleapis.com/css?family=Montserrat:300,400,500,600&display=swap" rel="stylesheet">
    <style>
        body, html{
            margin: 0;
            padding: 0;
            font-family: 'Montserrat', sans-serif;
        }

        .hero{
            position: relative;
        }

        .hero-image{
            width: 100%;
            height: auto;
        }

        .hero-text{
            position: absolute;
            right: 0;
            bottom: 0;
            left: 0;
            padding: 5rem 2.5rem;
            text-align: center;
            color: #fff;
            background-image: linear-gradient(rgba(0,0,0,0), rgba(0,0,0,.5));
        }

        .hero-text h1{
            font-weight: 300;
            font-size: 3rem;
            margin: 0 0 .5rem 0;
        }

        .slides{
            background: #424242;
            padding: 4px;
            display: flex
        }

        .slide{
            flex: 1;
            margin: 8px;
            border-radius: 4px;
            overflow: hidden;
            box-shadow: 0.3em 0.3em 1em rgba(0,0,0,0.3);
        }

        .slide img{
            width: 100%;
            height: auto;
        }

        .about{
            max-width: 1040px;
            margin: 0 auto;
        }

        .about h2{
            font-weight: 300;
            font-size: 2.5rem;
            margin: 2.5rem 0;
            text-align: center;
        }

        .about-block{
            display: flex;
            margin-bottom: 1.5rem;
        }

        .about-block:nth-child(odd) {
            flex-direction: row-reverse;
        }

        .about-block img{
            margin-right: 2.5rem;
        }
        
        .about-block:nth-child(odd) img {
            margin-left: 2.5rem;
            margin-right: 0;
        }

        .about-block .content{
            display: flex;
            flex-direction: column;
            justify-content: center;
        }

        .about-block .content h3{
            font-weight: 300;
            font-size: 2rem;
            margin: 0 0 1rem 0;
        }

        .about-block .content p{
            margin: 0;
            font-size: 1.2rem;
            line-height: 2;
        } 
    </style>
</head>
<body>
    <section class="hero">
        <img id="hero-image" class="hero-image" src="https://picsum.photos/seed/hero/1040/780">
        <div class="hero-text">
            <h1>Awesome Photo Gallery</h1>
            <p class="lead">Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
        </div>
    </section>
    <section class="slides">
        <div class="slide" onclick="switchImage('https://picsum.photos/seed/slide1')">
            <img src="https://picsum.photos/seed/slide1/240">
        </div>
        <div class="slide" onclick="switchImage('https://picsum.photos/seed/slide2')">
            <img src="https://picsum.photos/seed/slide2/240">
        </div>
        <div class="slide" onclick="switchImage('https://picsum.photos/seed/slide3')">
            <img src="https://picsum.photos/seed/slide3/240">
        </div>
        <div class="slide" onclick="switchImage('https://picsum.photos/seed/slide4')">
            <img src="https://picsum.photos/seed/slide4/240">
        </div>
        <div class="slide" onclick="switchImage('https://picsum.photos/seed/slide5')">
            <img src="https://picsum.photos/seed/slide5/240">
        </div>
    </section>
    <section class="about">
        <h2>About Us</h2>
        <div class="about-block">
            <img src="https://picsum.photos/seed/about1/420">
            <div class="content">
                <h3>About Block Headline</h3>
                <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras congue laoreet rutrum. In semper eget mauris vel congue. 
                    Quisque lacinia est nibh, sed sollicitudin leo mollis sit amet. Aliquam mollis dapibus porttitor. Vivamus rutrum 
                    augue nec nunc suscipit fermentum. Duis at neque lorem. Aliquam at luctus ex, eu molestie ligula.</p>
            </div>
        </div>
        <div class="about-block">
            <img src="https://picsum.photos/seed/about2/420">
            <div class="content">
                <h3>About Block Headline</h3>
                <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras congue laoreet rutrum. In semper eget mauris vel congue. 
                    Quisque lacinia est nibh, sed sollicitudin leo mollis sit amet. Aliquam mollis dapibus porttitor. Vivamus rutrum 
                    augue nec nunc suscipit fermentum. Duis at neque lorem. Aliquam at luctus ex, eu molestie ligula.</p>
            </div>
        </div>
        <div class="about-block">
            <img src="https://picsum.photos/seed/about3/420">
            <div class="content">
                <h3>About Block Headline</h3>
                <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras congue laoreet rutrum. In semper eget mauris vel congue. 
                    Quisque lacinia est nibh, sed sollicitudin leo mollis sit amet. Aliquam mollis dapibus porttitor. Vivamus rutrum 
                    augue nec nunc suscipit fermentum. Duis at neque lorem. Aliquam at luctus ex, eu molestie ligula.</p>
            </div>
        </div>
    </section>
    <script type="text/javascript">
        // preload images to avoid delay when you switch the src
        for (let i = 1; i <= 5; i++) {
            const image = new Image();
            image.src = 'https://picsum.photos/seed/slide' + i + '/1040/780';
            console.log('pre-loading ' + image.src);
        }
        
        const switchImage = (image) => {
            // get a reference to the hero image element
            const hero = document.getElementById("hero-image");

            // now load a full resolution version  
            const large = image + '/1040/780';

            // and set the hero image src
            hero.src = large;
        }
    </script>
</body>
</html>
```

## 1.3.3 Responsive design

![lesson 3.3 screenshot](screenshots/3.3.png)

* explain responsive design
* review page on mobile in chrome console
* add viewport meta tag
* add media query block for mobile devices
    * discuss mobile first development
* switch about block flex direction to column
* style images
    * make stretch to full width
* style content
    * add padding

```html
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta content="width=device-width, initial-scale=1" name="viewport" />
    <title>Photo Gallery</title>
    <meta name="description" content="Some beautiful stock photography"/>
    <link href="https://fonts.googleapis.com/css?family=Montserrat:300,400,500,600&display=swap" rel="stylesheet">
    <style>
        body, html{
            margin: 0;
            padding: 0;
            font-family: 'Montserrat', sans-serif;
        }

        .hero{
            position: relative;
        }

        .hero-image{
            width: 100%;
            height: auto;
        }

        .hero-text{
            position: absolute;
            right: 0;
            bottom: 0;
            left: 0;
            padding: 5rem 2.5rem;
            text-align: center;
            color: #fff;
            background-image: linear-gradient(rgba(0,0,0,0), rgba(0,0,0,.5));
        }

        .hero-text h1{
            font-weight: 300;
            font-size: 3rem;
            margin: 0 0 .5rem 0;
        }

        .slides{
            background: #424242;
            padding: 4px;
            display: flex
        }

        .slide{
            flex: 1;
            margin: 8px;
            border-radius: 4px;
            overflow: hidden;
            box-shadow: 0.3em 0.3em 1em rgba(0,0,0,0.3);
        }

        .slide img{
            width: 100%;
            height: auto;
        }

        .about{
            max-width: 1040px;
            margin: 0 auto;
        }

        .about h2{
            font-weight: 300;
            font-size: 2.5rem;
            margin: 2.5rem 0;
            text-align: center;
        }

        .about-block{
            display: flex;
            margin-bottom: 1.5rem;
        }

        .about-block:nth-child(odd) {
            flex-direction: row-reverse;
        }

        .about-block img{
            margin-right: 2.5rem;
        }
        
        .about-block:nth-child(odd) img {
            margin-left: 2.5rem;
            margin-right: 0;
        }

        .about-block .content{
            display: flex;
            flex-direction: column;
            justify-content: center;
        }

        .about-block .content h3{
            font-weight: 300;
            font-size: 2rem;
            margin: 0 0 1rem 0;
        }

        .about-block .content p{
            margin: 0;
            font-size: 1.2rem;
            line-height: 2;
        } 

        @media (max-width: 375px) {
            .about-block{
                display: block;
            }

            .about-block img, .about-block:nth-child(odd) img{
                margin: 0;
                width: 100%;
                height: auto;
            }

            .about-block .content{
                display: block;
                padding: 1.5rem;
            }

            .about-block .content h3{
                font-weight: 400;
                font-size: 1.5rem;
                margin: 0 0 1rem 0;
            }

            .about-block .content p{
                font-size: .9rem;
                line-height: 1.5;
            } 
        }
    </style>
</head>
<body>
    <section class="hero">
        <img id="hero-image" class="hero-image" src="https://picsum.photos/seed/hero/1040/780">
        <div class="hero-text">
            <h1>Awesome Photo Gallery</h1>
            <p class="lead">Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
        </div>
    </section>
    <section class="slides">
        <div class="slide" onclick="switchImage('https://picsum.photos/seed/slide1')">
            <img src="https://picsum.photos/seed/slide1/240">
        </div>
        <div class="slide" onclick="switchImage('https://picsum.photos/seed/slide2')">
            <img src="https://picsum.photos/seed/slide2/240">
        </div>
        <div class="slide" onclick="switchImage('https://picsum.photos/seed/slide3')">
            <img src="https://picsum.photos/seed/slide3/240">
        </div>
        <div class="slide" onclick="switchImage('https://picsum.photos/seed/slide4')">
            <img src="https://picsum.photos/seed/slide4/240">
        </div>
        <div class="slide" onclick="switchImage('https://picsum.photos/seed/slide5')">
            <img src="https://picsum.photos/seed/slide5/240">
        </div>
    </section>
    <section class="about">
        <h2>About Us</h2>
        <div class="about-block">
            <img src="https://picsum.photos/seed/about1/420">
            <div class="content">
                <h3>About Block Headline</h3>
                <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras congue laoreet rutrum. In semper eget mauris vel congue. 
                    Quisque lacinia est nibh, sed sollicitudin leo mollis sit amet. Aliquam mollis dapibus porttitor. Vivamus rutrum 
                    augue nec nunc suscipit fermentum. Duis at neque lorem. Aliquam at luctus ex, eu molestie ligula.</p>
            </div>
        </div>
        <div class="about-block">
            <img src="https://picsum.photos/seed/about2/420">
            <div class="content">
                <h3>About Block Headline</h3>
                <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras congue laoreet rutrum. In semper eget mauris vel congue. 
                    Quisque lacinia est nibh, sed sollicitudin leo mollis sit amet. Aliquam mollis dapibus porttitor. Vivamus rutrum 
                    augue nec nunc suscipit fermentum. Duis at neque lorem. Aliquam at luctus ex, eu molestie ligula.</p>
            </div>
        </div>
        <div class="about-block">
            <img src="https://picsum.photos/seed/about3/420">
            <div class="content">
                <h3>About Block Headline</h3>
                <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras congue laoreet rutrum. In semper eget mauris vel congue. 
                    Quisque lacinia est nibh, sed sollicitudin leo mollis sit amet. Aliquam mollis dapibus porttitor. Vivamus rutrum 
                    augue nec nunc suscipit fermentum. Duis at neque lorem. Aliquam at luctus ex, eu molestie ligula.</p>
            </div>
        </div>
    </section>
    <script type="text/javascript">
        // preload images to avoid delay when you switch the src
        for (let i = 1; i <= 5; i++) {
            const image = new Image();
            image.src = 'https://picsum.photos/seed/slide' + i + '/1040/780';
            console.log('pre-loading ' + image.src);
        }
        
        const switchImage = (image) => {
            // get a reference to the hero image element
            const hero = document.getElementById("hero-image");

            // now load a full resolution version  
            const large = image + '/1040/780';

            // and set the hero image src
            hero.src = large;
        }
    </script>
</body>
</html>
```

6. update blocks so they display on mobile devices
